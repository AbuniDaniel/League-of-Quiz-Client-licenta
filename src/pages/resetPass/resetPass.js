...
function ResetPass() {
  ...
  useEffect(() => {
    const verifyEmailUrl = async () => {
      ...
      const response = await Axios.get(
        `https://licenta-server-production.up.railway.app/users/${param.id}/reset-pass?token=${forgotToken}`
      );
  ...
  const resetPassF = async (values, actions) => {
    const response = await Axios.post("https://licenta-server-production.up.railway.app/reset-password", {
        id: param.id,
